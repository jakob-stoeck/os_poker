...
class socketDaemon {
  ...
  public $socket_select_timeout = 3;
	...
	public function process()
	{
		...
		$event_time    = time();
		while (($events = socket_select($read_set, $write_set, $exception_set, $this->socket_select_timeout)) !== false) {
			if ($events > 0) {
			...
			if (time() - $event_time > $this->socket_select_timeout) {
				// only do this if more than socket_select_timeout passed, else we'd keep looping this for every bit recieved
        foreach ($this->servers as $socket) {
          $socket->on_timer();
        }
				foreach ($this->clients as $socket) {
