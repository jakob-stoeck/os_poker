...
function os_inviteFriendsDisplayer (data,container) {
	console.log(data);
	...
	for(itemId in data.friends.list) {
		...
		html +=	'<div class="buddy_pager>"'
			...
			+	'<div class="picture">'
					+'<a title="Spielerprofil ansehen." href="javascript:parent.os_shareApp(\''+item.id+'\')"><img width="118" height="118" title="'+item.displayName+' Profilbild" alt="'+item.thumbnailUrl+' Profilbild" src="'+item.thumbnailUrl+'"></a>'
				+'</div>'
			+'</div>'
			+'<div class="buddy_result_list_infos">'
			+	'<div class="buddy_result_list_name ">'
			+		'<a class="thickbox" href="javascript:parent.os_shareApp(\''+item.id+'\')">'+item.displayName+'</a>'
			+	'</div>'
			+'</div>'
			+'<div class="buddy_result_list_links">'
			+'<div onclick="javascript:parent.os_shareApp(\''+item.id+'\')" class="link_invite poker_submit silver">'
				+'<div class="pre"> </div>'
